...
if [ -n "$ggmlcuda" ]; then
    ...
    if curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install_v2.sh | bash -s -- -v $wasmedge_version $args_wasmedge --tmpdir=$tmp_dir --ggmlcuda=$ggmlcuda; then
    ...
    info "* Install without cuda... -v 0.13.5 --noavx"
    if curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install_v2.sh | bash -s -- -v 0.13.5 --noavx; then
        source $HOME/.wasmedge/env
        wasmedge_path=$(which wasmedge)
        info "    * The $wasmedge_version is installed in $wasmedge_path."
    else
